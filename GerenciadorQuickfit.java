...
public class GerenciadorQuickfit {
    ...
    private int ultimoSegmentoAlocado = 0; // Índice do último segmento alocado

    public GerenciadorQuickfit(int tamanhoUnidadeAlocacao, int tamanhoMemoria) {
    ...
    public void alocaProcesso(Processo p) {
        ...
        if (indice == -1) {
            ...
            ultimoSegmentoAlocado = (indice + 1) % listaEncadeada.size(); // Atualiza o índice do último segmento alocado
        }
    }
    ...
    public int buscaSegmento(int comprimento) {
        int inicioBusca = ultimoSegmentoAlocado;
        for (int a = inicioBusca; a < listaEncadeada.size(); a++) {
            if (!listaEncadeada.get(a).isOcupado() && listaEncadeada.get(a).getComprimento() >= comprimento) {
                return a;
            }
        }
        for (int a = 0; a < inicioBusca; a++) {
            if (!listaEncadeada.get(a).isOcupado() && listaEncadeada.get(a).getComprimento() >= comprimento) {
                return a;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        GerenciadorQuickfit gerenciador = new GerenciadorQuickfit(2, 62);
